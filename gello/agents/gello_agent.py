...
PORT_CONFIG_MAP: Dict[str, DynamixelRobotConfig] = {
    #xArm6
    "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTA2U4SR-if00-port0": DynamixelRobotConfig(
        joint_ids=(1, 2, 3, 4, 5, 6),
        joint_offsets=(
        4*np.pi/2, 3*np.pi/2, -1*np.pi/2, 0*np.pi/2, 4*np.pi/2, 4*np.pi/2
        ),
        joint_signs=(1, 1, -1, 1, -1, 1),
        gripper_config=(7, 17.378125, -24.421875),
    ),
    # panda
